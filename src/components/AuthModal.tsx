  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "auth/invalid-credential" || err.code === "auth/user-not-found" || err.code === "auth/wrong-password") {
        ...
        msg = isArabic
          ? "طريقة تسجيل الدخول بالبريد الإلكتروني غير مفعّلة في وحدة تحكم Firebase (Firebase Console > Authentication > Sign-in method)."
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === "auth/operation-not-allowed" || err.message?.includes("operation-not-allowed")) {
        setError(isArabic
          ? "تسجيل الدخول عبر Google غير مفعّل في وحدة تحكم Firebase (Firebase Console > Authentication > Sign-in method)."
        ...
        {currentUser ? (
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3 px-4 rounded-xl bg-white border-2 border-slate-200 hover:border-emerald-500 hover:bg-slate-50 text-slate-800 font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition disabled:opacity-50"
            >
